...
public final class FontGlyph {
    private static final int KERNING_KEY = (0x09);
    private static final int KERNING_PAGE_SIZE = (0x01 << KERNING_KEY);
    private static final int KERNING_PAGE = (0x10000 / KERNING_PAGE_SIZE);
    ...
    public void addKerning(int character, int amount) {
        final int index = character >>> KERNING_KEY;

        if (mKerning == null) {
            mKerning = new byte[KERNING_PAGE][];
        }

        byte[] page = mKerning[index];

        if (page == null) {
            page = mKerning[index] = new byte[KERNING_PAGE_SIZE];
        }
        page[character & KERNING_PAGE_SIZE - 1] = (byte) amount;
    }
    ...
    public int getKerning(int character) {
        ...
        if (mKerning != null) {
            return (mKerning[index] != null ? mKerning[index][character & KERNING_PAGE_SIZE - 1] : 0);
        }
        return 0;
    }
}
