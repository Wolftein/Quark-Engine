...
public final class TextureDDSAssetLoader implements AssetLoader<Texture, Texture.Descriptor> {
     ...
     */
    @Override
    public AssetKey<Texture, Texture.Descriptor> load(
            AssetManager manager, Texture.Descriptor descriptor, InputStream input) throws IOException {
    ...
    private Image readImage(Texture.Descriptor descriptor, ImageHeader header, DataInputStream input) throws IOException {
        final FastList<Image.Layer> layer = new FastList<>(header.mImageDepth);
        ...
        for (int i = 0; i < header.mImageDepth; ++i) {
            ...
            //! Create the buffer.
            //!
            final Int8Buffer buffer
                    = BufferFactory.allocateInt8(imageLength).write(content, 0, imageLength).flip();
        ...
        return new Image(format, header.mImageWidth, header.mImageHeight, header.mImageDepth, layer.toImmutable());
    }
    ...
    private ImageFormat getUncompressedFormat(ImageHeader header) throws IOException {
        final boolean hasAlpha = (header.mPixelFormatFlags & DDPF_ALPHA_PIXELS) != 0;

        if ((header.mImageFlag & DDPF_RGB) != 0) {
            return hasAlpha ? ImageFormat.RGBA : ImageFormat.RGB;
        } else if ((header.mImageFlag & DDPF_ALPHA) != 0 || (header.mImageFlag & DDPF_GRAY_SCALE) != 0) {
            return hasAlpha ? ImageFormat.RG : ImageFormat.RED;
        }
    ...
    private ImageFormat getCompressedFormat(ImageHeader header) throws IOException {
        ...
        switch (header.mPixelFormatFourCC) {
            case 0x44585431:
                return hasAlpha ? ImageFormat.RGBA_DXT1 : ImageFormat.RGB_DXT1;
            case 0x44585433:
