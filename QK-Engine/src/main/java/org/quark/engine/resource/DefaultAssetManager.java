...
/**
 * <code>DefaultAssetManager</code> represent a default implementation for {@link AssetManager}.
 *
...
public final class DefaultAssetManager implements AssetManager {
    ...
    private final Map<String, AssetKey> mCache = new ConcurrentHashMap<>();
    ...
    public void removeAssetLocator(String name) {
        if (mLocators.remove(name) == null) {
            throw new IllegalStateException(
                    "An AssetLocator with the provided name doesn't exist.");
        }
    }
    ...
    public void removeAssetLoader(String... extensions) {
        for (final String extension : extensions) {
            if (mLoaders.remove(extension) == null) {
                throw new IllegalStateException(
                        "An AssetLoader with the provided extension doesn't exist.");
            }
        }
    }
     ...
     */
    @Override
    public <A, B extends AssetDescriptor> A loadAsset(String filename, B pDescriptor) throws IOException {
        //!
        //! Tries to find the Asset in the cache.
        //!
    ...
    public void unloadAllAssets() {
        mCache.values().forEach(AssetKey::onKeyDispose);
        mCache.clear();
    }
}
