 ...
 */
package org.quark.engine.mathematic.camera;

import org.quark.engine.mathematic.*;

import java.util.concurrent.atomic.AtomicBoolean;

/**
 * <code>Camera</code> represent a camera with position and rotation.
 *
...
public class Camera {
     ...
     */
    protected MutableMatrix4f mCombined = MutableMatrix4f.createIdentity();
    ...
    public Matrix4f getView() {
        if (mDirty0.compareAndSet(true, false)) {
            mView = mTemp0.setRotation(mRotation).mul(mTemp1.setTranslation(mPosition));
        }
        return mView;
    }
    ...
    public Matrix4f getCombined() {
        if (mDirty1.compareAndSet(true, false)) {
            mCombined.set(mProjection).mul(mView);
        }
        return mCombined;
    }
